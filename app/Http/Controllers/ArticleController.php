...
class ArticleController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        //$this->sendMail($articles, $request);
        $articles->save();
        $users = new User();
    ...
    public function sendMail($id)
    {
        ...
        */
        $article=Article::findOrfail($id);
        $journaliste= User::findOrFail($article->iduser);
        if($journaliste->role=='Coordonnateur Journal tabloïd'){
            Mail::send('emails.send_article', ['articles' => $article], function ($message) use ($article){
                $journaliste= User::findOrFail($article->iduser);
                $user = User::where('role','infographe')->first();
                $message->from($journaliste->email, "Envoie de l'article  {{$article->titre}} ");
                $message->to($user->email);
                $message->subject("Envoie de l'article  {$article->titre} ");
            });
        }elseif($journaliste->role=='Coordonnateur Journal en ligne'){
            Mail::send('emails.send_article', ['articles' => $article], function ($message) use ($article){
                $journaliste= User::findOrFail($article->iduser);
                $user = User::where('role','webmaster')->first();
                $message->from($journaliste->email, "Envoie de l'article  {{$article->titre}} ");
                $message->to($user->email);
                $message->subject("Envoie de l'article  {$article->titre} ");
            });
